...
export const formatCount = count => {
    if(count) {
        // count = 2 --> 2; count = 2.5 --> 2 1/2; count = 0.5 --> 1/2;
        const [int, dec] = count.toString().split('.').map(el => parseInt(el, 10));

        if(!dec) {   // count = 2 --> 2; 
            return count;
        } 
        // count = 1.33333333333 --> 1.3
        const fixCount = count.toFixed(1);
        if(int === 0) {  //count = 0.5 --> 1/2;
            const fr = new Fraction(fixCount);
            return `${fr.numerator}/${fr.denominator}`;

        } else {   //count = 2.5 --> 2 1/2;
            const fr = new Fraction(fixCount - int);
...
export const renderRecipeDetail = (recipe, isLiked) => {
    const markup = `
        ...
        <div class="recipe__ingredients">
            ...
            <button class="btn-small recipe__btn btn-add-to-list">
...
export const updateServings = recipe => {
    ...
    const elementList = document.querySelectorAll('.recipe__count');
